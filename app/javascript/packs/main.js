...
const modalUser = document.querySelector('.modal-user'); // Получаю модальное окно данных пользователя
const closeUser = document.querySelector('.close-user'); // Получаю кнопку закрытия модального окна данных пользователя
...
function authorized() { // Функция, которая выполняется, когда пользователь авторизован 
  ...
  function logOut() { // Функция, диавторизующая пользователя
    ...
    // userName.removeEventListener("click", toggleModalUser);
    // closeUser.removeEventListener('click', toggleModalUser);
    checkAuth(); // Вызываю функцию проверки авторизации
    returnMain(); // Вызываю функцию возврата на страницу ресторанов
  }
  closeUser.addEventListener('click', toggleModalUser);
  // userName.addEventListener('click', toggleModalUser);
